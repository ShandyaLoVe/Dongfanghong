//	
//	      OLED_Show3FNum(0,0, Ultra_B_L,3,3,12);       //3  Ç°
//	  OLED_Show3FNum(60,0, Ultra_F_L,3,3,12);      //4    ×ó
...
{
    /*ÈëÂ¢*/
	if(Move_flag==2)start_1(0,850);//85
	else if(Move_flag==3)start_2(0,900);//85
	else if(Move_flag==4)start_3(0,600);//75
...
//    
//	/*»»Â¢1*/
	else if(Move_flag==10)tran_1(0,-650); //600
//    
	/*¿çÂ¢2*/
	else if(Move_flag==12)second_1(-800,0); //800
...
void start_3(int vx,int vy)
{
	...
	mike_wheel(vx+out2,vy,out1);
	Solve_EncoData();
	if(Ultra_F_L<228.0f&&Ultra_F_L>30.0f)  // used 208.0f
	{
...
void first_1(int vx,int vy)
{
	...
	out2=ultra_base1_Pid_Ctrl(&ultra_base1_pid,Ultra_F_L,140.0f);
...
void first_3(int vx,int vy)
{
	...
	Solve_EncoData();
	if(Ultra_B_R<150.0f&&Ultra_B_R>10.0f) //170  /10
	{
        filter(15);
		Encoder_Clear();
...
void tran_1(int vx,int vy)
{
	...
	mike_wheel(vx-out2,vy,out1);
	Solve_EncoData();
	if((my_abs(Encoders.disA)>125.0f||my_abs(Encoders.disB)>125.0f)&&Ultra_B_L>300.0f) //130 130 300
	{
			Encoder_Clear();
		    state=1;
...
void second_1(int vx,int vy)
{
	...
	TRIG_1
	TRIG_3
  decrease_speed_1(&vx,0.33); //  0.22
	out1=yaw_base1_Pid_Ctrl(&yaw_base2_pid,yaw,start_yaw);
	mike_wheel(vx,vy,out1);
	Solve_EncoData();
	if(my_abs(Encoders.disA)>65.0f||my_abs(Encoders.disB)>65.0f) //70 70
	{
		mike_wheel(0,0,0);
		Encoder_Clear();
		Move_flag=13;
...
void third_3(int vx,int vy)
{
	...
	mike_wheel(vx,vy,out1);
	Solve_EncoData();
	if(Ultra_B_R<200.0f&&Ultra_B_R>10.0f)
	{
        filter(15);
		Encoder_Clear();
		USART_SendData(USART1,2);
...
void tran_3(int vx,int vy)
{
	...
	mike_wheel(vx-out2,vy,out1);
	Solve_EncoData();
	if((my_abs(Encoders.disA)>85.0f||my_abs(Encoders.disB)>85.0f)&&Ultra_B_L>300.0f) /// 80 80 90
	{
			Encoder_Clear();
		    state=1;
	}
...
void tran_4(int vx,int vy)
{
	...
	if((my_abs(Encoders.disA)>85.0f||my_abs(Encoders.disB)>85.0f)&&Ultra_B_R>300.0f) //80 80 300
	{
			Encoder_Clear();
		    state=1;
	}
...
void tran_5(int vx,int vy)
{
	...
	mike_wheel(vx-out2,vy,out1);
	Solve_EncoData();
	if(Ultra_F_R<200.0f&&Ultra_F_R>10.0f)
...
void sixth_3(int vx,int vy)
{
	...
	mike_wheel(vx,vy,out1);
	Solve_EncoData();
	if(Ultra_B_L<175.0f&&Ultra_B_L>10.0f) ////200
	{
		mike_wheel(0,0,0);
        filter(15);
		Encoder_Clear();
