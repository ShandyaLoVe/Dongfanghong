...
{
    /*ÈëÂ¢*/
	if(Move_flag==2)start_1(0,850);//85//	if(Move_flag==2)start_1(0,900);//85
//	if(Move_flag==2)start_1(0,0);//85//	if(Move_flag==2)start_1(0,900);//85
	
	else if(Move_flag==3)start_2(0,900);//85
	else if(Move_flag==4)start_3(0,650);//75

    /*¿çÂ¢1*/
	else if(Move_flag==6)first_1(700,0);
	else if(Move_flag==7)first_2(1000,0);//1
	else if(Move_flag==8)first_3(850,0);
//    
	...
	/*¿çÂ¢2*/
	else if(Move_flag==12)second_1(-900,0);
	else if(Move_flag==13)second_2(-1000,0);//2
	else if(Move_flag==14)second_3(-850,0);
    
	/*»»Â¢2*/
	else if(Move_flag==16)tran_2(0,-600);
    
	/*¿çÂ¢3*/
	else if(Move_flag==18)third_1(900,0);
	else if(Move_flag==19)third_2(1000,0);
	else if(Move_flag==20)third_3(850,0);
    
	/*»»Â¢3*/
	else if(Move_flag==22)tran_3(0,-600);
	
    /*¿çÂ¢4*/
	else if(Move_flag==24)fourth_1(-900,0);
	else if(Move_flag==25)fourth_2(-1000,0);
	else if(Move_flag==26)fourth_3(-850,0);
    
	/*»»Â¢4*/
	else if(Move_flag==28)tran_4(0,-600);
	
    /*¿çÂ¢5*/
	else if(Move_flag==30)fifth_1(900,0);
	else if(Move_flag==31)fifth_2(1000,0);
	else if(Move_flag==32)fifth_3(850,0);
	...
	else if(Move_flag==38)sixth_3(-850,0);
...
//	TRIG_2
	...
	Solve_EncoData();
	if(my_abs(Encoders.disA)>100.0f||my_abs(Encoders.disB)>100.0f)
	{
		mike_wheel(0,0,0);
		Encoder_Clear();
     	Move_flag=3;
...
void start_2(int vx,int vy)
{
	...
	out2=ultra_base1_Pid_Ctrl(&ultra_base1_pid,Ultra_B_L,80.0f);
	mike_wheel(vx+out2,vy,out1);
	Solve_EncoData();
	if(my_abs(Encoders.disA)>650.0f||my_abs(Encoders.disB)>650.0f)
	{
		mike_wheel(0,0,0);
		Move_flag=4;
	}
...
void start_3(int vx,int vy)
{
	...
	mike_wheel(vx+out2,vy,out1);
	Solve_EncoData();
	if(Ultra_F_L<140.0f&&Ultra_F_L>10.0f)   //240 10
	{
...
void first_1(int vx,int vy)
{
	...
	out2=ultra_base1_Pid_Ctrl(&ultra_base1_pid,Ultra_F_L,85.0f);
//	increase_speed(&vx,1000);
...
void first_2(int vx,int vy)
{
	...
	mike_wheel(vx,vy+out2,out1);
	Solve_EncoData();
	if(my_abs(Encoders.disA)>460.0f||my_abs(Encoders.disB)>460.0f)
	{
		mike_wheel(0,0,0);
		Encoder_Clear();
		Move_flag=8;
...
void first_3(int vx,int vy)
{
	...
	mike_wheel(vx,vy-out2,out1);
	Solve_EncoData();
	if(Ultra_B_R<230.0f&&Ultra_B_R>10.0f)
	{
        filter(15);
		Encoder_Clear();
...
void tran_1(int vx,int vy)
{
	...
	mike_wheel(vx-out2,vy,out1);
	Solve_EncoData();
	if((Ultra_B_L>350.0f)&&(my_abs(Encoders.disA)>115.0f||my_abs(Encoders.disB)>115.0f)&&Ultra_B_L>300.0f) //130 130 300
	{
			Encoder_Clear();
		    state=1;
    ...
    if(state==1&&(my_abs(Encoders.disA)>12.0f||my_abs(Encoders.disB)>12.0f))
	{
			mike_wheel(0,0,0);
			filter(15);
			Encoder_Clear();
...
void second_1(int vx,int vy)
{
	...
	Solve_EncoData();
	if(my_abs(Encoders.disA)>70.0f||my_abs(Encoders.disB)>70.0f)
	{
		mike_wheel(0,0,0);
		Encoder_Clear();
		Move_flag=13;
...
void tran_2(int vx,int vy)
{
	...
	if((Ultra_B_R>350.0f)&&(my_abs(Encoders.disA)>80.0f||my_abs(Encoders.disB)>80.0f))
	{
			Encoder_Clear();
		    state=1;
	}
...
void third_3(int vx,int vy)
{
	...
	mike_wheel(vx,vy,out1);
	Solve_EncoData();
	if(Ultra_B_R<230.0f&&Ultra_B_R>10.0f) //200   10
	{
        filter(15);
		Encoder_Clear();
//		USART_SendData(USART1,2);
...
void tran_3(int vx,int vy)
{
	...
	mike_wheel(vx-out2,vy,out1);
	Solve_EncoData();
	if((Ultra_B_L>350.0f)&&(Ultra_B_L>500.0f)&&(my_abs(Encoders.disA)>80.0f||my_abs(Encoders.disB)>80.0f))
	{
			Encoder_Clear();
		    state=1;
	}
...
void tran_4(int vx,int vy)
{
	...
	if((Ultra_B_R>350.0f)&&(my_abs(Encoders.disA)>80.0f||my_abs(Encoders.disB)>80.0f)&&Ultra_B_R>300.0f)
	{
			Encoder_Clear();
		    state=1;
	}
...
void tran_5(int vx,int vy)
{
	...
	mike_wheel(vx-out2,vy,out1);
	Solve_EncoData();
	if(Ultra_F_R<210.0f&&Ultra_F_R>10.0f)
...
void sixth_3(int vx,int vy)
{
	...
	mike_wheel(vx,vy,out1);
	Solve_EncoData();
	if(Ultra_B_L<200.0f&&Ultra_B_L>10.0f)
	{
		mike_wheel(0,0,0);
        filter(15);
		Encoder_Clear();
...
void back(int vx,int vy)
{
	...
	out1=yaw_base1_Pid_Ctrl(&yaw_base1_pid,yaw,start_yaw);
	out2=ultra_base1_Pid_Ctrl(&ultra_base1_pid,Ultra_B_L,68.0f);
	mike_wheel(vx+out2,vy,out1);
	Solve_EncoData();
	if(my_abs(Encoders.disA)>70.0f||my_abs(Encoders.disB)>70.0f)
	{
...
void back_2(int vx,int vy)
{
	...
	mike_wheel(vx,vy,out);
	Solve_EncoData();
	if(my_abs(Encoders.disA)>28.0f||my_abs(Encoders.disB)>28.0f)
	{
...
void increase_speed(int *vx,int goat)
{
	static int speed=0;
	int state=1;
	if(goat < 0)
