...
u8 rec_data_u2[U2_DATA_LEN];      													//接收数据地址
u8 send_data_u2[U2_DATA_LEN];     													//发送数据地址
...
void USART2_Init(int Baud)
{
  ...
  USARTx_CLK_INIT(RCC_APB2PeriphClockCmd, ENABLE);								//打开串口对应的外设时钟
...
void USART2_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USARTx, USART_IT_IDLE) != RESET)  											//接收中断
	{
		...
		//USART2_Rec_Len = U1_DATA_LEN - USART_RX_DMA_Stream->NDTR;
		//***********帧数据处理函数************//
...
{ 
	...
	USART_RX_DMA_Stream->NDTR = U2_DATA_LEN;            
...
{
    sprintf(send,"%f,%f\n",data1,data2);//转化为char
    USART2_DMATransfer((u32*)send,strlen(send));
}
