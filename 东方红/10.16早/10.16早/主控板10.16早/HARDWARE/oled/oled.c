...
#define MOSI_GPIO_CLK           RCC_AHB1Periph_GPIOB

#define CLK_PIN                  GPIO_Pin_12
#define CLK_GPIO_PORT            GPIOB
#define CLK_GPIO_CLK             RCC_AHB1Periph_GPIOB
...
{     
	...
	dzk=(u8*)Chinese[no];	//得到汉字编号对应的点阵库 
	for(t=0;t<csize;t++)
	{   												   
		temp=dzk[t];//得到点阵数据                          
		for(t1=0;t1<8;t1++)
...
{ 	 		 
  ...
  RCC_AHB1PeriphClockCmd(CS_GPIO_CLK|DC_GPIO_CLK|MOSI_GPIO_CLK|CLK_GPIO_CLK,ENABLE);                          //使能GPIOB时钟
  ...
  GPIO_Init(CS_GPIO_PORT, &GPIO_InitStructure);																				//初始化
  ...
  GPIO_Init(CLK_GPIO_PORT, &GPIO_InitStructure);	
	//初始化
	GPIO_ResetBits(CS_GPIO_PORT,CS_PIN);//GPIOF9,F10éè????￡?μ??e
	GPIO_ResetBits(DC_GPIO_PORT,DC_PIN);//GPIOF9,F10éè????￡?μ??e
	GPIO_ResetBits(MOSI_GPIO_PORT,MOSI_PIN);//GPIOF9,F10éè????￡?μ??e
	GPIO_ResetBits(CLK_GPIO_PORT,CLK_PIN);//GPIOF9,F10éè????￡?μ??e
